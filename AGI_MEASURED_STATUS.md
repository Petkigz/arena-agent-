...
- ✅ **Owner-sovereign autonomy**: Autonomous limits are optional and disabled by default. The owner may create persistent directive goals/tasks, inspect the complete queue, approve/reject planning, reprioritize work, and separately request execution. Goal approval never authorizes actions. A persistent run ledger records observation, consideration, recommendation, planning approval, execution, blocking, budget stops, and outcomes per cycle/goal. Exact sovereign grants may override owner policy, while emergency pause, physical availability, critical resources, payload integrity, and evidence honesty remain absolute.
