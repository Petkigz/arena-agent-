...
3. Extend cooperative cancellation into remaining blocking third-party HTTP and browser automation calls. Subprocess-heavy package, device, git, diagnostics, update, and sandbox tools now terminate process groups, but synchronous library calls without an interruptible API still finish before cancellation is observed.
