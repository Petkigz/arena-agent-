...
- ✅ **Owner-sovereign autonomy**: Autonomous resource limits are optional and disabled by default, so Arena is not arbitrarily restricted. The owner may enable persistent caps for duration, cooldown, executions, project work, and failures. Separately, an authenticated owner can issue a short-lived exact-payload sovereign grant that overrides their own blocklist, mode, or safety ceiling. It cannot bypass emergency pause, critical resource shutdown, missing capabilities, exact payload binding, or evidence honesty.
