...
- ✅ **Owner-sovereign autonomy**: Limits are optional and disabled by default. The owner may create persistent directives, inspect the queue, approve/reject planning, reprioritize, and separately request execution. A timezone-aware persistent calendar atomically claims one-time/daily/weekly directives, uses deterministic occurrence IDs, recovers stale claims, and advances to the next future local wall time. Each autonomous cycle holds one expiring SQLite lease, preventing overlapping scheduler/manual runs. Owner priority is authoritative in queue selection: critical/high directives preempt higher-scored normal/low goals before execution begins. Goal/schedule approval never authorizes actions. A run ledger records observation, consideration, recommendation, planning approval, execution, blocking, budgets and outcomes, with authorization, controlled-execution, trace, verification and rollback receipt links per executed step. Multi-goal allocation makes owner priority dominant, excludes incomplete dependencies, and uses CPU/RAM pressure only among comparable ready work. Approved plan revisions persist environment assumptions—owner policy, tool contracts/availability, interfaces, capability count, and goal priority—and fail closed across sessions when those assumptions drift. Urgent owner goals can preempt through cancellation receipts. Execution results persist; observation-only reconciliation binds the exact reviewed step and recommends skip/wait/replan without replay. Resume is separate, blocked without reconciliation, and blocked while evidence remains unknown. Exact sovereign grants may override owner policy. The explicit `allow_sensitive_autonomy` switch can also delegate manifest Level-3 actions by raising the ceiling to 3; disabling it immediately reclamps to 2. Emergency pause, physical availability, OS privilege, critical resources, payload integrity, and evidence honesty remain absolute.
...
- ✅ **P2 Resource-aware planning**: Hardware tiers are live-probed rather than fixed at 16 GB. A 32GB+ / 20-thread host receives the high-memory profile; 40GB+ raises verified-memory consolidation from 100 to 500 episodes and retained-record cap from 5,000 to 20,000, recommends up to six read-oriented CPU workers, and considers 14B Q4 CPU inference while retaining 9B speed. Pressure remains percentage-based and critical 95–98% stops remain.
...
- ✅ **Simulated-success removal**: Screenshot capture, desktop input, Win32 HWND operations, browser navigation, TTS, research digests, SVG generation, pairing/onboarding, wake-word training, and speaker identification fail or report unavailable instead of fabricating outcomes. Pentest reports require explicit scope/findings; an empty list produces a labeled zero-findings template rather than invented vulnerabilities or placeholder CVEs. LLM-offline responses carry `success=false`/`simulated=true`, and conversational ANSWER goals defer rather than becoming verified from placeholder text. Cross-domain similarity is now `predicted_success` with `verified=false`; it cannot update transfer success history until external evidence is recorded.
