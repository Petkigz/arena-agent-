...
- ✅ **Evidence-linked functional self-knowledge and commitments**: A revisioned SQLite ledger records hardware, capability, authority, and limitation claims only with source type, evidence, confidence, timestamp, and freshness. Contradicting observations supersede rather than erase prior beliefs. Agency attribution distinguishes self-caused, owner-caused, external, and unknown changes; temporal proximity alone remains unknown, while `self_caused` requires a controlled execution plus environmental observation and goal-verifying evidence. A separate restart-safe commitment ledger reconciles persistent projects and exact authorized actions; completion requires verification evidence and blocked work retains a reason. Introspection summaries are assembled from persisted trace facts and explicitly deny access to hidden chain-of-thought. This is operational introspection, not consciousness.

- ✅ **Longitudinal intelligence regression history**: An isolated deterministic suite now measures 12 checks: paraphrased memory retrieval, success/failure utility adaptation, outcome-calibrated autonomy, evidence-linked consolidation, authorization replay resistance, temporal continuity, LoRA review boundaries, project dependency unlocking, owner curiosity limits, conservative agency attribution, and restart-safe commitment continuity. Each run persists per-check evidence/metrics/duration and reports pass→fail regressions against the previous run. It reports factual pass counts, never an “AGI percentage,” and does not mutate the live brain.
