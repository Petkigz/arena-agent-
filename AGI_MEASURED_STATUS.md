...
| Tools in the manifest | **148** | `len(get_tool_manifest())` — added detect_objects, detect_faces, analyze_image_grounded, analyze_prosody, vlm_analyze, vlm_status, list_loras, lora_status, activate_lora, deactivate_lora, prepare_lora_dataset, create_lora_job (P1-1, P2, P3) |
...
User / WebSocket chat (now multimodal: text + image_path + attachments)
  → CognitiveRuntime.process_cognitive_cycle(image_path, attachments)
      ...
      → capability execution (148 manifest tools)
...
- **tools_wired** — 148 tools registered in the capability registry (from the manifest) — was 118
