...
- ✅ **Native Owner Control operations**: Desktop and Android expose policy mode, autonomous safety ceiling, exploration cap, emergency pause/resume, exact recommendation approval/rejection, revision-bound exact plan-step JSON editing, plan approval/rejection, separate approved-plan execution, active authorization execution/revocation, execution cancellation, and rollback-request creation. Reviewed grants expose their original payload only when it still hashes to the immutable authorization digest; direct grants never invent a payload. Both native clients explicitly preserve authorization versus execution, report goal verification separately from tool success, and keep rollback as a new approval.
