...
- ✅ **Owner-bounded autonomy envelope**: Autonomous cycles now obey persistent owner caps for total duration, cooldown, goal executions, project count/steps, and consecutive failures. Emergency pause or disabled cycles skip before observation/execution; observe/suggest modes may generate recommendations but cannot auto-approve or execute them. Zero execution/project budgets remain zero and never broaden existing authority.
