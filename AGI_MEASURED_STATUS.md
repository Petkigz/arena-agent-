...
- ✅ **Cooperative execution control + rollback receipts**: Every runtime capability execution now receives a persistent execution ID, cancellation state, and rollback receipt. Owner stop requests are visible while work runs; ToolRegistry checks cancellation around handlers, and disposable sandbox commands use cancellable `Popen` process groups rather than blocking `subprocess.run`. Cancellation arriving after the final checkpoint is reported honestly because side effects may exist. Rollback is never implied: unsupported actions receive a reason, while supported compensation creates a new exact approval request rather than auto-running.
...
3. Extend cooperative cancellation into remaining blocking HTTP, package-install, browser, and device tools. Controlled execution history, owner stop requests, sandbox process-group termination, and truthful rollback receipts now exist, but third-party synchronous calls without checkpoints still finish before cancellation is observed.
