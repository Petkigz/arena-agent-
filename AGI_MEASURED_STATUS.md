...
- ✅ **Owner-sovereign autonomy**: Autonomous resource limits are optional and disabled by default, so Arena is not arbitrarily restricted. The owner may create persistent directive goals/tasks, inspect the complete autonomous goal queue, approve or reject planning, reprioritize work, and separately request execution of the next goal. Goal approval never authorizes actions. For exact commands, an authenticated owner can issue a short-lived payload-bound sovereign grant overriding their own mode/block/level policy; emergency pause, physical availability, critical resources, payload integrity, and evidence honesty remain absolute.
