...
- ✅ **Native Owner Control operations**: Desktop and Android now expose policy mode, autonomous safety ceiling, exploration cap, emergency pause/resume, exact recommendation approval/rejection, plan approval/rejection, separate approved-plan execution, execution cancellation, and rollback-request creation. Android displays exact pending-action payloads; desktop exposes complete selected records as JSON. Both explicitly state that authorization does not execute, plan approval does not execute, and rollback creates a new approval rather than running compensation.
...
7. Add exact step/payload plan editing and scoped-authorization execution to the native Android and desktop interfaces. Both now cover operational policy, exact-action approval, plan decision/separate execution, cancellation, and rollback controls; the web remains the complete editing surface.
