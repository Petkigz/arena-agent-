...
| 0.3 | Add a bounded longitudinal evaluation runner for repeated benchmark behavior. | **DONE — IMPLEMENTED AND WIRED for deterministic contract trends** | The isolated benchmark has 29 deterministic checks, including six explicitly scoped held-out grounding/outcome cases, plus persisted pass/fail trend reporting and an API endpoint; held-out task outcome effects remain bounded replay evidence, not a generalization or AGI score. |
...
| 1.4 | Calibrate confidence and response usefulness over longitudinal held-out tasks. | **IN PROGRESS / PARTIAL** | Recorded-outcome calibration, unsupported-claim control, correction recovery, and paired held-out outcome/usefulness replays are benchmarked; real task correction speed, usefulness volume, and broader held-out improvement remain to be measured. |
...
1. **Extend Phase 1 held-out evaluation (1.4).** The isolated suite now covers 29 deterministic contracts, including recorded calibration trends, unsupported-claim/correction/empty-observation controls, and paired outcome/usefulness replays. Next add repeated real task runs that measure correction speed, usefulness volume, route choice, memory compounding, and style changes without treating synthetic replay as generalization evidence.
