...
| 0.3 | Add a bounded longitudinal evaluation runner for repeated benchmark behavior. | **DONE — IMPLEMENTED AND WIRED for deterministic contract trends** | The isolated benchmark has 32 deterministic checks, including eight explicitly scoped held-out grounding/outcome/correction cases, plus persisted pass/fail trend reporting and owner-visible evidence APIs; held-out task outcome effects remain bounded replay evidence, not a generalization or AGI score. |
...
| 1.4 | Calibrate confidence and response usefulness over longitudinal held-out tasks. | **IN PROGRESS / PARTIAL** | Recorded-outcome calibration, unsupported-claim control, correction recovery, correction receipt/latency telemetry, paired held-out outcome/usefulness replays, and an owner-visible aggregate report are implemented; broader real-task usefulness volume and outcome improvement remain unverified. |
...
1. **Extend Phase 1 held-out evaluation (1.4).** The isolated suite now covers 32 deterministic contracts, including recorded calibration trends, unsupported-claim/correction/empty-observation controls, correction telemetry, paired outcome/usefulness replays, and aggregate evidence reporting. Next collect repeated real-task runs through the evidence endpoint for usefulness volume and outcome improvement without treating synthetic replay as generalization evidence.
