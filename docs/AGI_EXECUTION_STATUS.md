...
- Full repository suite (2026-09-07, latest — includes the 36-check intelligence benchmark and in-chat correction coverage): **3,213 passed, 18 skipped, 3 warnings**.
...
| 0.3 | Add a bounded longitudinal evaluation runner for repeated benchmark behavior. | **DONE — IMPLEMENTED AND WIRED for deterministic contract trends** | The isolated benchmark has 36 deterministic checks, including eleven explicitly scoped held-out grounding/outcome/correction/memory-compounding cases, plus persisted pass/fail trend reporting and owner-visible evidence/evaluation APIs; held-out task outcome effects remain bounded replay evidence, not a generalization or AGI score. |
