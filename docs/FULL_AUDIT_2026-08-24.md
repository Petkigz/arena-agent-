...
suite unchanged at 1783 passed. Step 10b: 28 OS-grounding / browser / desktop / raw-input routes (including browser service adapters) and their 14 request models moved verbatim to app/api/os_browser_automation.py; main.py is now ~2.8k lines. The move surfaced and fixed a real latent bug: /automation/browser/disk-status reached BrowserAutomation through the lazy proxy, which cannot resolve the DOWNLOADS_DIR class attribute (and the probe target was not created on fresh checkouts) — the endpoint now uses the concrete class and mkdirs before probing, with a regression test. Step 10c: all 12 /self-awareness routes (snapshot, claims history, belief revisions, agency, identity checkpoint, recovery list/decision/action-approval, embodied boundary, commitments get/post, grounded introspection) and 4 request models moved verbatim to app/api/self_awareness.py; main.py is now ~2.67k lines; moved names re-exported from app.main. Step 10d (slice 1): CognitiveRuntime._integrate_phase_modules (~270 lines of phase-module wiring) extracted verbatim to app/cognition/runtime_wiring.py as integrate_phase_modules(rt, ...); the runtime delegates with itself as the single authoritative instance — no second runtime, no state, no public-surface change. runtime.py: ~3.44k → ~3.17k lines. Full suite identical. Remaining slices: capability probes (measure_capabilities), authorized-execution orchestration, and the cognitive-cycle method — these need true refactoring, not verbatim moves, and continue in separate verified increments.
