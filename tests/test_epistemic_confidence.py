...
def test_ambiguous_collision_verb_triggers_semantic_interpretation():
    """
    Ambiguous collision verbs (e.g., 'find', 'check') trigger deeper semantic interpretation
    to disambiguate intent between operational action, information need, and knowledge query.
    """
    mock_llm_reply = {
        "choices": [{
            "message": {
                "content": '{"primary_intent_type": "information_need", "target_domain": "diagnostic", "goal": "Investigate why Photoshop crashed", "desired_outcome": "Crash evidence gathered"}'
            }
        }]
    }

    with patch("app.llm.llm_client.generate_chat_completion", return_value=mock_llm_reply):
        goal_rep = SemanticGoalInterpreter.interpret_goal("find out why Photoshop crashed", complexity="fast")
        assert goal_rep.primary_intent_type == "information_need"
        assert goal_rep.target_domain == "diagnostic"
        assert goal_rep.confidence == 0.95 or goal_rep.confidence == 0.85
        assert goal_rep.provenance_source in ["llm_heuristic_agreement", "llm_semantic_disambiguation"]
