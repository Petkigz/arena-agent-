...
def test_conversation_messages_roundtrip(clean_db):
    ...
    # message_id (the SQLite row id) rides along so every UI can dedupe
    # hydrated history against live token streams.
    assert [(m["role"], m["content"]) for m in messages] == [
        ("user", "hello"),
        ("assistant", "hi there"),
    ]
    assert all(isinstance(m["message_id"], int) for m in messages)
...
def test_router_history_survives_restart(clean_db, monkeypatch):
    ...
    # Roles/content survive the restart; message_id is the DB row id after a
    # reload (live entries generated by the router use msg_* ids instead).
    assert [(m["role"], m["content"]) for m in history] == [
        ("user", "remember me"),
        ("assistant", "ok, remembered"),
    ]
    assert all("message_id" in m for m in history)
