...
def test_semantic_activation_uses_observed_bounds(tmp_path,monkeypatch):
 ...
 s=AccessibilityStore(tmp_path/'a.db');s.ingest([{'role':'button','name':'Submit','bounds':{'x':0,'y':0,'width':100,'height':40}}],interface='test',window_id='w1',evidence=['native tree'])
 monkeypatch.setattr(AccessibilityControlTool,'store',s)
 monkeypatch.setattr(AccessibilityControlTool,'os_grounding',SimpleNamespace(get_by_window=lambda wid:SimpleNamespace(grounding_id='osg_test',pid=42,window_id='w1')))
 topo={'success':True,'topology_sha256':'d'*64,'monitors':[{'display_id':'display_0','x':0,'y':0,'width':1920,'height':1080}]}
 with patch('app.tools.deep_os_controller.DeepOSController.mouse_click',return_value={'success':True}) as click,\
      patch.object(DisplayTopologyTool,'capture',return_value=topo):
  r=AccessibilityControlTool.activate_target('button','Submit')
 assert r['success'] is True and r['semantic_target_verified'] is True
 click.assert_called_once_with(50,20,grounding_id='osg_test',expected_topology_sha256='d'*64,fresh_observation=ANY)
 fresh=click.call_args.kwargs['fresh_observation']
 assert fresh['age_seconds']<=10 and fresh['evidence'][0].startswith('accessibility_snapshot:')
