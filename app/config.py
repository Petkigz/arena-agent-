...
class Settings(BaseSettings):
    ...
    ARENA_EVENT_LEDGER: str = "1"  # Phase 1 (owner go-ahead 2026-09-11): the typed Event ledger — one event id per owner request, typed states (new→dispatched→observation_pending→verified_success/verified_failure/superseded/abandoned/expired), append-only receipts, legacy-trace backfill. "0" silences every entry point. Read via os.environ like its siblings.
    ARENA_REPLY_CRITIC: str = "1"  # Round 9 (owner go-ahead 2026-09-13): the critic pass — the fast lane reads each draft reply against the executed evidence and appends a visible correction when a claim outruns what actually ran (semantic lies the regex honesty guard cannot know). "0" disables. Read via os.environ like its siblings.
    ARENA_CONTEXT_DISTILL: str = "1"  # Round 10 (owner go-ahead 2026-09-13): context distillation — turns leaving the 16-message window are compressed into a standing labeled digest injected ahead of the recent turns; re-distilled at most every 8 departures; a simulated model never fabricates memory. "0" disables. Read via os.environ like its siblings.
    ARENA_EMBED_CACHE: str = "1"  # Round 11 (owner go-ahead 2026-09-13): persistent embedding cache — vectors the provider actually computed survive restarts and timeouts, so a dead embedder degrades to REAL cached vectors instead of fuzzy strings; rescue is all-or-nothing and never fabricates. "0" disables. Read via os.environ like its siblings.
    ARENA_SELF_IMPROVEMENT: str = "1"  # Gated self-improvement loop (owner go-ahead 2026-09-13): evidence-only weakness ranking (the Chooser), append-only experiment records on the Phase 1 ledger, and the OWNER GATE — measured improvements wait for approval, nothing auto-applies, the benchmark evaluator is read-only to the improver. "0" disables every entry point. Read via os.environ like its siblings.
