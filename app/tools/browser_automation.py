...
class BrowserAutomation:
    ...
    def navigate_and_extract(
        ...
        has_submit_step = any(
            str(s.get("action") or s.get("verb") or "").lower().strip() == "submit"
            for s in (steps or [])
        )
        if submit_form or has_submit_step:
        ...
        try:
            ...
            with sync_playwright() as p:
                ...
                # P0 #6: an ordered step list executes SEQUENTIALLY (each
                # step sees the page state left by the previous one). The
                ...
                if not steps and click_selectors:
            ...
            return {
                ...
                # present they carry more signal than the raw body text.
                "content_snippet": ("\n\n".join(extracts.values()) or page_text)[:5000],
                "extracts": extracts,
                "step_log": step_log,
