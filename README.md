...
│   ├── tools/            # 139 capability tools (manifest) — vision grounding + VLM + LoRA + prosody
...
Current software-only baseline (2026-08-23): **1589 backend passed, 2 skipped, 4 e2e deselected; 184 frontend passed; production frontend build passed**. The measured architecture retains 27/27 deterministic scorecard checks, 139 manifest tools, and 17 wired cognition modules.
...
requires a unique accessibility role/name match with observed screen bounds. Current
privilege and process provenance are exposed at `/os-control/privilege` and
`/os-control/process-ownership/{pid}`.
